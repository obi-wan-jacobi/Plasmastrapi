        ...
        if (inputState === null) {
            for (var i = 0, L = connections.length; i < L; i++) {
                if (connections[i].state.isPowered) {
                    isPowered = true;
                    nextState = (nextState || connections[i].state) && !(nextState && connections[i].state);
                }
            }
            if (!isPowered) {
            ...
            if (!this.outputTerminal.isPowered) {
                ...
                this.outputTerminal.state = (this.outputTerminal.state || inputState) && !(this.outputTerminal.state && inputState);
                return;
            }
        }
    };
